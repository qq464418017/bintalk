...
bool Options::parseOptions(int argc, char* argv[])
{
	...
	for(int i = 1; i < argc; i++)
	{
		...
		else if (::strcmp(opt, "-jp") == 0)
		{
			GET_OPT_PARAM(javaPackagePath_);
		}
		else if (::strcmp(opt, "-gp") == 0)
		{
			GET_OPT_PARAM(goPackagePath_);
		}
		else if (::strcmp(opt, "-dp") == 0)
		{
			GET_OPT_PARAM(dartPackagePath_);
		}
		else
			return false;
	}
