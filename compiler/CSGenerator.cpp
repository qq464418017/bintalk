...
static void generateEnum(CodeFile& f, Enum* e)
{
	...
	f.indent("{");
	f.output("public static bool read(bintalk.BReader r, ref %s v, uint maxValue)", e->getNameC());
	f.indent("{");
...
static void generateStruct(CodeFile& f, Struct* s)
{
	...
	f.recover("}");
	// deserialize code.
	f.output("public bool deserialize(bintalk.BReader __r__)");
	f.indent("{");
	...
	f.output("public static partial class ProtocolReader");
	f.indent("{");
	f.output("public static bool read(bintalk.BReader r, ref %s v, uint maxValue)", s->getNameC());
	f.indent("{");
